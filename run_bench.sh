#!/usr/bin/env bash
#
# run_bench.sh -- micro-benchmark runner for the PHP binding.
# Builds libitb.so via build.sh, then runs bench/bench.php:
...
# bench/bench.php reads ITB_MSG_PROFILE for the Message shape and
...
exec php "${PHP_ARGS[@]}" bench/bench.php
